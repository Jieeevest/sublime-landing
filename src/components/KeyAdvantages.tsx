...
export default function KeyAdvantages() {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-teal-800 to-teal-900 text-white relative overflow-hidden">
      {/* Background decoration */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-teal-600 rounded-full opacity-10 blur-3xl"></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 bg-teal-700 rounded-full opacity-10 blur-3xl"></div>
          ...
          <div className="space-y-8">
            <div>
              ...
              <p className="text-teal-100 text-lg">
                Experience the difference with our comprehensive approach to audio-based healing and wellness.
              </p>
            </div>
            ...
            <ul className="space-y-4">
              {advantages.map((advantage, index) => (
                <li key={index} className="flex items-start gap-3">
                  ...
                  <span className="text-lg text-teal-50">{advantage}</span>
                </li>
              ))}
            </ul>
          </div>
