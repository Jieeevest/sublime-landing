...
export default function PaymentFinishPage() {
    ...
    useEffect(() => {
        // Midtrans sends these query params to the finish URL
                    ...
                    {status === "success" && (
                        <div className="w-20 h-20 rounded-full bg-[#D3E8EB] flex items-center justify-center mb-6">
                            <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
                                <path d="M33 12L16.5 28.5L8 20" stroke="#3197A5" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" />
                            </svg>
                        </div>
                    )}
                    {status === "pending" && (
                        <div className="w-20 h-20 rounded-full bg-amber-50 flex items-center justify-center mb-6">
                            <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
                                ...
                                <path d="M20 13v8l5 3" stroke="#F59E0B" strokeWidth="3" strokeLinecap="round" />
                            </svg>
                        </div>
                    )}
                    {status === "failed" && (
                        <div className="w-20 h-20 rounded-full bg-red-50 flex items-center justify-center mb-6">
                            <svg width="40" height="40" viewBox="0 0 40 40" fill="none">
                                <path d="M12 12l16 16M28 12L12 28" stroke="#EF4444" strokeWidth="3.5" strokeLinecap="round" />
                            </svg>
                        </div>
                    )}
                    ...
                    {orderId && (
                        <div className="w-full bg-[#F3F8F9] rounded-xl p-4 mb-8 text-left space-y-2">
                            <div className="flex justify-between text-sm">
                                ...
                                <span className="text-[#1F1F1F] font-medium font-mono text-xs">{orderId}</span>
                            </div>
                            {transactionId && (
                                <div className="flex justify-between text-sm">
                                    <span className="text-gray-500">Transaction ID</span>
                                    <span className="text-[#1F1F1F] font-medium font-mono text-xs">{transactionId}</span>
                                </div>
                            )}
                            <div className="flex justify-between text-sm">
                                <span className="text-gray-500">Status</span>
                                <span
                                    className={`font-semibold text-xs px-2 py-0.5 rounded-full ${status === "success"
                                ...
                                >
                                    {status === "success" ? "Berhasil" : status === "pending" ? "Menunggu" : "Gagal"}
                                </span>
                            </div>
                        </div>
                    )}
